...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ClassLink's recommended GUID: TenantId + SourcedId. TenantId scopes every
    // other identifier to one district; SourcedId is the stable per-user GUID.
    ...
    const SELECT =
      'id, email, account_type, classlink_login_id, classlink_tenant_id, classlink_sourced_id';

    // Resolve a returning Quest user on ClassLink's recommended identifier
    // precedence (these are the fields submitted for certification):
    //   1. (TenantId + SourcedId)  — the recommended GUID
    //   2. (TenantId + LoginId)    — LoginId is unique only within a tenant
    //   3. LoginId                 — legacy rows provisioned before tenant existed
    ...
    if (tenantId && sourcedId) {
      const { data } = await admin.from('users').select(SELECT)
        ...
        .eq('classlink_sourced_id', sourcedId).maybeSingle();
      existing = data ?? null;
    }
    ...
    // by GUID/LoginId and the ClassLink email has since changed).
