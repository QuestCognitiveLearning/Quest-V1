    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="w-64 bg-[#1E40AF] text-white flex flex-col border-r border-[#1E40AF]" style={{fontFamily: '"Inter", sans-serif'}}>
        ...
        <nav className="flex-1 overflow-y-auto py-2 px-2">
          <button onClick={() => paid && handleNavigation("generate", "Generate")} className={itemClass(activeNav === "generate")}>
            <Sparkles className="w-4 h-4" />
            <span>Generate</span>
          </button>
          <button onClick={() => paid && handleNavigation("dashboard", "TeacherDashboard")} className={itemClass(activeNav === "dashboard")}>
            <BarChart3 className="w-4 h-4" />
            <span>Dashboard</span>
          </button>
          <button onClick={() => paid && handleNavigation("curricula", "TeacherCurricula")} className={itemClass(activeNav === "curricula")}>
